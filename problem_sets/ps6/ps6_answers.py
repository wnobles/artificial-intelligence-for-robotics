    ...
    for measurement, motion in data:
        ...
        for meas in measurement:
            ...
            for b in range(2):
                ...
                Xi.value[b][0] += -meas[1+b] / measurement_noise
                Xi.value[m+b][0] += meas[1+b] / measurement_noise
        ...
        new_idxs = list(range(2, len(Omega.value)))
        a = Omega.take([0, 1], new_idxs)
        b = Omega.take([0, 1])
        c = Xi.take([0, 1], [0])
        Omega = Omega.take(new_idxs) - a.transpose() * b.inverse() * a
        Xi = Xi.take(new_idxs, [0]) - a.transpose() * b.inverse() * c
