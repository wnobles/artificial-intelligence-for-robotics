...
def q4_move(self, motion):
    ...
    # get the steering angle and vehicle distance
    steering, distance = motion

    # instantiate a robot object
    res = robot()

    # set the robot's attributes
    ...
    turning_angle = distance / res.length * tan(steering)

    # if executing motion in a straight line...
    if abs(turning_angle) < 0.001:
        res.x = self.x + distance * cos(self.orientation)
        res.y = self.y + distance * sin(self.orientation)
        res.orientation = (self.orientation + turning_angle) % (2 * pi)

    # otherwise, the motion is cyclic...
    else:
...
def q5_sense(self, add_noise=1):
    ...
    # The code below is adapted from:
    # https://gatech.instructure.com/courses/402244/pages/5-sensing-answer?module_item_id=4089026
    # Particle Filters: Sensing (Answer)

    for landmark in landmarks:
    
        # get the orientation in global coordinates and adjust for the robot's local position
...
def q6_move(self, motion):
    ...
    # introduce noise
    steering_angle_with_noise = random.gauss(steering, self.steering_noise)
    distance_with_noise = random.gauss(distance, self.distance_noise)

    # get the turning angle
    turning_angle = distance_with_noise / res.length * tan(steering_angle_with_noise)
    ...
    # otherwise, the motion is cyclic...
        ...
        radius = distance_with_noise / turning_angle
        cx = self.x - radius * sin(self.orientation)
        cy = self.y + radius * cos(self.orientation)
        res.orientation = (self.orientation + turning_angle) % (2 * pi)
        res.x = cx + radius * sin(res.orientation)
