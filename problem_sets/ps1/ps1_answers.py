...
q3_nonnormalized_P_F_given_B     = (1 - q3_P_L) * q3_P_F
q3_nonnormalized_P_not_F_given_B = q3_P_L * (1 - q3_P_F)
...
def sense(p, colors, measurement, sensor_right):
    ...
    for i, row in enumerate(p):
        ...
        for j, element in enumerate(row):
            ...
            # store whether the color matches the measurement
            hit = measurement == colors[i][j]

            # multiply the prior by the likelihood
            joint_prob = element * (hit * sensor_right + (1-hit) * (1-sensor_right))

            # store the non-normalized probabilities in each row
            new_row.append(joint_prob)

        # store each row
        q.append(new_row)

    # compute the sum
    norm_factor = sum(sum(row) for row in q)

    # normalize the matrix values
    q = [[elem / norm_factor for elem in row] for row in q]
...
def move(p, motions, p_move):
    ...
    for i, row in enumerate(p):
        ...
        for j, element in enumerate(row):
            ...
            shift = p_move * (p[(i-motions[0]) % len(p)][(j-motions[1]) % len(p[i])]) + (1-p_move) * element

            # store the non-normalized probabilities in each row
            new_row.append(shift)
